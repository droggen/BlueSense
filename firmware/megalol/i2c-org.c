...
void TWI_vect_intx(void)
{
	...
	if(_i2c_transaction_idle==1)
	{
		...
		_i2c_current_transaction->dodata=_i2c_current_transaction->dodata;	// Cache dodata to speedup access
...
*/
	...
	switch(_i2c_current_transaction_state)							
	{
		...
		case 2:		
			...
			//_i2c_current_transaction_n=0;										// Setups the number of bytes currently transferred			// DAN: this was already set to zero before first calling the vector
		...
		case 3:
			...
			if(_i2c_current_transaction->rw==I2C_READ)
			{
				if(!_i2c_current_transaction->extdata)
					...
					_i2c_current_transaction->extdata[_i2c_current_transaction_n-1]=TWDR;
			}
		...
		case4:
			...
			if(_i2c_current_transaction_n<_i2c_current_transaction->dodata)			// If data to transmit data
			{
				...
				{
					if(!_i2c_current_transaction->extdata)
						TWDR = _i2c_current_transaction->data[_i2c_current_transaction_n];
					else
						TWDR = _i2c_current_transaction->extdata[_i2c_current_transaction_n];
					TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWIE);										
					#ifdef I2CDBG
						//uart0_putchar('x');
						//uart0_putchar('w');
				...
				else
				{
					...
					if(_i2c_current_transaction_n == _i2c_current_transaction->dodata-1)	// Last data
						TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWIE);												// Clear acknowledge bit
					else
						TWCR = (1<<TWINT)|(1<<TWEN)|(1<<TWIE)|(1<<TWEA);							// Set acknowledge bit
